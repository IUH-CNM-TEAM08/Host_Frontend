...
export default function FriendRequestList() {
                ...
                if (!senderNames[senderId] || !senderAvatars[senderId]) {
                    try {
                        ...
                        if (response.success && response.user) {
                            setSenderNames(prev => ({
                                ...prev,
                                [senderId]: response.user?.name || t('contacts.unknownUser')
                            }));
    ...
    const loadFriendRequests = async () => {
        try {
            ...
            setError(t('contacts.loadFriendRequestsError'));
    ...
    const handleSearch = async (query: string) => {
        ...
        try {
            ...
            if (response.success && response.users && response.users.length > 0) {
                ...
                setSearchResults([]);
                Alert.alert(t('common.error'), t('contacts.noUsersFound'));
            }
        } catch (err) {
            Alert.alert(t('common.error'), t('contacts.cannotSearchUsers'));
    ...
    return (
        <View className="flex-1">
                                    ...
                                    {isFriend ? (
                                        <View key={`search-friend-${result.id}`} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                                            <View style={{ backgroundColor: '#f3f4f6', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 999 }}>
                                                <Text style={{ color: '#374151', fontWeight: '500' }}>{t('contacts.alreadyFriend')}</Text>
                                            </View>
                                            <TouchableOpacity
                                        ...
                                        pendingRequestSent.status === "declined" ? (
                                            <View key={`search-declined-${result.id}`} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                                                <View style={{ backgroundColor: '#fee2e2', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 999 }}>
                                                    <Text style={{ color: '#dc2626', fontWeight: '500' }}>{t('contacts.requestRejected')}</Text>
                                                </View>
                                                <TouchableOpacity
                                            ...
                                            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                                                ...
                                                >
                                                    <Text style={{ color: '#fff', fontWeight: '500' }}>{t('contacts.recallRequest')}</Text>
                                                </TouchableOpacity>
                                                <TouchableOpacity
                                                    style={{ backgroundColor: '#6366f1', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 999 }}
                                                    onPress={() => handleStartChat(result.id)}
                                        ...
                                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                                            ...
                                            >
                                                <Text style={{ color: '#fff', fontWeight: '500' }}>{t('contacts.acceptRequest')}</Text>
                                            </TouchableOpacity>
                                            <TouchableOpacity
                                            ...
                                            >
                                                <Text style={{ color: '#374151', fontWeight: '500' }}>{t('contacts.rejectRequest')}</Text>
                                            </TouchableOpacity>
                                            <TouchableOpacity
                                                style={{ backgroundColor: '#6366f1', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 999 }}
                                                onPress={() => handleStartChat(result.id)}
            ...
            <ScrollView className="flex-1">
                <Text className="px-4 py-2 text-sm font-semibold text-gray-500">{t('contacts.friendRequests')}</Text>
                {requests.length === 0 ? (
                    <View key="no-requests" className="items-center justify-center p-4">
                        <Ionicons name="people-outline" size={48} color="#666" />
                        <Text className="text-gray-500 mt-2">{t('contacts.friendRequests')}</Text>
                    </View>
                ) : (
                    requests.map((request) => (
                        ...
                        >
                            ...
                            <View key={`request-info-${request.id}`} className="flex-1 ml-3">
                                <Text className="font-medium text-gray-900">
                                    {senderNames[request.senderId] || t('common.loading')}
                                </Text>
                                <Text className="text-sm text-gray-500">
                            ...
                            <View key={`request-actions-${request.id}`} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                                {request.status !== "declined" ? (
                                    <>
                                        ...
                                        >
                                            <Text style={{ color: '#fff', fontWeight: '500' }}>
                                                {startingChatUserId === request.senderId ? t('common.loading') : t('contacts.sendMessage')}
                                            </Text>
                                        </TouchableOpacity>
                                    </>
                                ) : (

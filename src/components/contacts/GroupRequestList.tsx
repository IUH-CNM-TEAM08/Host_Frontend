        ...
        try {
            ...
            setInvites(
                (data?.invites ?? []).map((inv: any) => ({
                    ...
                    group: inv.group ?? { id: inv.conversationId, name: 'Nhóm', memberCount: 0 },
    ...
    const handleAccept = async (invite: GroupInvite) => {
        ...
        try {
            ...
            if (pendingAdmin) {
                ...
                Alert.alert('Thành công', `Đã tham gia nhóm ${invite.group.name}`);
            }
        } catch (e: any) {
            const msg =
    ...
    if (loading) {
        return (
            <View className="flex-1 items-center justify-center py-10">
                ...
                <Text className="text-gray-400 mt-3">Đang tải lời mời...</Text>
            </View>
        );
    }
    ...
    if (invites.length === 0) {
        return (
            <View className="flex-1 items-center justify-center p-4 py-16">
                ...
                <Text className="text-gray-400 mt-3 text-base">Không có lời mời tham gia nhóm</Text>
                <TouchableOpacity
                    onPress={() => void fetchInvites()}
                    className="mt-4 px-4 py-2 rounded-full border border-blue-200 bg-blue-50"
                >
                    <Text className="text-blue-500 text-sm">Làm mới</Text>
                </TouchableOpacity>
            </View>
        );
    }
                        ...
                        <View className="flex-row items-center">
                            ...
                            <View className="flex-1 ml-3">
                                ...
                                <Text className="text-sm text-gray-500 mt-0.5">
                                    {invite.group.memberCount} thành viên
                                </Text>
                            ...
                            {invite.awaitingAdminApproval ? (
                                ...
                                >
                                    {actionLoading === invite.id ? (
                                        ...
                                        <Text className="text-gray-600 font-medium">Huỷ yêu cầu</Text>
                                    )}
                                </TouchableOpacity>
                            ) : (
                                <>

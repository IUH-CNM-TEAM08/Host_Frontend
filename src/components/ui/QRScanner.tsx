...
export default function QRScanner({
    ...
    const handleLoginQr = async (qrId: string) => {
        try {
            ...
            Alert.alert(t('qr.successTitle'), t('qr.successMessage'));
            setShowQRScanner && setShowQRScanner(false);
        } catch (error: any) {
            Alert.alert(t('qr.confirmErrorTitle'), extractErrorMessage(error));
        } finally {
            setTimeout(() => {
    ...
    const handlePickImage = async () => {
        ...
        try {
            ...
            Alert.alert(t('common.error'), t('qr.galleryError') + (error?.message || t('common.unknownError')));
        }
    };
    ...
    const decodeQRCode = async (uri: string) => {
        ...
        try {
            ...
            if (decodedData) {
                ...
                Alert.alert(t('qr.notFoundTitle'), t('qr.notFoundMessage'));
                qrLock.current = false;
            }
        } catch (error: any) {
            console.error('[QRScanner] Decode process error detail:', error);
            Alert.alert(t('common.error'), t('qr.decodeError') + (error?.message || t('common.unknownError')));
            qrLock.current = false;
        }
    };
    ...
    if (!permission) {
        return (
            <View style={styles.permissionContainer}>
                <Text style={styles.permissionText}>{t('qr.checkPermission')}</Text>
            </View>
        );
    }
    ...
    if (permission.status !== 'granted') {
        return (
            <View style={styles.permissionContainer}>
                <Text style={styles.permissionText}>{t('qr.requestPermission')}</Text>
                <Text
    ...
    if (!isMounted) {
        return (
            <View style={styles.permissionContainer}>
                <Text style={styles.permissionText}>{t('qr.connectingCamera')}</Text>
            </View>
        );
    }

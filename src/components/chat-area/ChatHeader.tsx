...
export default function ChatHeader({
  ...
  useEffect(() => {
    const handleUserProfileUpdated = (payload: {
      ...
      if (!targetId) return;
      const currentOtherId = String(otherUserIdRef.current ?? "");
      if (!currentOtherId || currentOtherId !== targetId) return;

      setOtherUser((prev) => {
        if (!prev) return prev;
        return {
          ...prev,
          name: String(payload?.displayName ?? "").trim() || prev.name,
          avatar: String(payload?.avatarUrl ?? "").trim() || prev.avatar,
        };
      });
    };

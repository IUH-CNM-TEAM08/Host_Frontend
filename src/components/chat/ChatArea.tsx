...
export default function ChatArea({
    ...
    if (!groupChat.canSendMessage) {
      setError(
        groupChat.lockedChatMessage ||
          "Bạn không có quyền nhắn tin trong nhóm này.",
      );
      return;
    }
  ...
  const handleConfirmEnableAi = async () => {
    ...
    try {
      ...
      if (res?.success === false) {
        setError("Không bật được Bot AI. Vui lòng thử lại.");
        return;
      }
      onConversationMetaChanged?.();
  ...
  const handleDeclineEnableAi = () => {
    ...
    setPendingAiMessage("");
    setToastMessage(
      "Bạn chưa bật AI, Bot AI chưa thể đọc tin nhắn. Mời bạn vào thông tin đoạn chat để bật.",
    );
  ...
  const handleForward = async (selectedConversations: string[]) => {
    ...
    if (
      ...
      setError("Không có tin nhắn hợp lệ để chuyển tiếp");
      return;
    }
    ...
    if (failedCount > 0 || skippedSourceCount > 0) {
      const parts = [`Đã chuyển tiếp ${successCount} tin`];
      if (failedCount > 0) parts.push(`${failedCount} tin lỗi`);
      if (skippedSourceCount > 0)
        parts.push(`${skippedSourceCount} tin không hợp lệ`);
      setError(parts.join(", "));
    }
  };
  ...
  const handleCreateVote = () => {
    ...
    if (voteDeadlineDate) {
      ...
      if (!deadlineIso) {
        const errorMsg =
          "Định dạng ngày/giờ hết hạn không hợp lệ (DD/MM/YYYY HH:MM)";
    ...
    const deadlineNotice = deadlineIso
      ? ` (Hết hạn lúc: ${formatAppointmentTime(deadlineIso)})`
      : "";
    void sendSystemNotice(
      `${actorDisplayName} đã tạo bình chọn: "${voteQuestion.trim()}"${deadlineNotice}`,
    );
                ...
                <View className="items-center mb-3">
                  ...
                  >
                    ...
                    <Text className="ml-2 text-[12px] text-gray-600 font-medium">
                      {loadingOlderMessages
                        ? "Đang tải dữ liệu cũ..."
                        : "Bạn có muốn load data cũ hơn không?"}
                    </Text>
                  </TouchableOpacity>
                </View>
              )}
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        <Text className="ml-2 text-gray-800">
                          {isRecordingVoice
                            ? `Dừng và gửi ghi âm (${voiceRecordingSeconds}s)`
                            : "Ghi âm thoại"}
                        </Text>
                      </TouchableOpacity>
          ...
          >
            ...
            >
              ...
              <Text style={{ fontSize: 14, color: "#4b5563", lineHeight: 20 }}>
                Bạn có muốn bật Bot AI ngay bây giờ để tiếp tục câu hỏi này
                không?
              </Text>
            </View>

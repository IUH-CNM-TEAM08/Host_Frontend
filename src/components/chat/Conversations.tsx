...
export default function Conversations({
            ...
            await Promise.all(
                [...needApiIds].map(async (id) => {
                    try {
                        ...
                        newNames[id] = t('chatArea.defaultUser');
                    } catch {
                        newNames[id] = t('chatArea.defaultUser');
                    }
                })
    ...
    useEffect(() => {
        ...
        const handleNewConversation = () => {
            // Khi vừa tạo nhóm hoặc vừa được thêm vào nhóm mới, refresh list ngay
            // để mobile không cần chờ focus lại màn hình.
    ...
    // Realtime profile update: chỉ patch cục bộ theo userId, không refetch toàn danh sách.
    useEffect(() => {
        const handleUserProfileUpdated = (payload: {
            ...
            if (nextName) {
                setParticipantNames((prev) => ({ ...prev, [targetId]: nextName }));
            }
            if (nextAvatar) {
                setParticipantAvatars((prev) => ({ ...prev, [targetId]: nextAvatar }));
            }

            const patchConversationList = (list: Conversation[]) =>
                list.map((conversation) => {
                    if (!conversation.participantIds?.includes(targetId)) return conversation;
                    const nextParticipantInfo = Array.isArray(conversation.participantInfo)
                        ? conversation.participantInfo.map((p: any) =>
                            String(p?.id) === targetId
                                ? {
                                    ...p,
                                    name: nextName || p?.name,
                                    displayName: nextName || p?.displayName,
                                    avatar: nextAvatar || p?.avatar,
                                }
                                : p
                        )
                    ...
                    return {
                        ...conversation,
                        participantInfo: nextParticipantInfo as any,
                    };
                });

            setConversations((prev) => patchConversationList(prev));
            setHiddenConversations((prev) => patchConversationList(prev));
            setStrangerConversations((prev) => patchConversationList(prev));
        };
    ...
    const lastMessagePreviewLabel = (conversation: Conversation): string => {
        const lm = conversation.lastMessage;
        if (!lm) return t('chat.noMessages');
        if (lm.isDeletedForEveryone) return t('chatArea.pollRevoked');
        ...
        if (typeNorm === MessageType.SYSTEM) {
            const messageText = String(lm.content ?? '').trim();
            return messageText || t('settings.notifications');
        }

        let prefix = "";
        if (lm.senderId === user?.id) {
            prefix = `${t('commentSheet.you')}: `;
        } else if (conversation.isGroup) {
            const senderName = participantNames[lm.senderId] || t('chatArea.defaultUser');
        ...
        if (lm.content != null && String(lm.content).trim()) {
            ...
            switch (lm.type) {
                case MessageType.IMAGE:
                    contentText = t('chatArea.image'); break;
                case MessageType.VIDEO:
                    contentText = 'Video'; break;
                case MessageType.FILE: {
                    const name = (lm.metadata as { fileName?: string } | undefined)?.fileName;
                    contentText = name ? `${t('chatArea.file')}: ${name}` : t('chatArea.file'); break;
                }
                case MessageType.AUDIO:
                    contentText = t('chatArea.voiceRecord'); break;
                case MessageType.CALL:
                    contentText = t('chatArea.callStarting'); break;
                case MessageType.VOTE:
                    contentText = t('chatArea.poll'); break;
                case MessageType.MEDIA_ALBUM:
                    contentText = t('chatArea.mediaAlbum'); break;
                default:
                    contentText = t('chatArea.message'); break;
            }
        }
    ...
    const handleUnlockHiddenConversation = async () => {
        ...
        if (!/^\d{4,6}$/.test(pin)) {
            Alert.alert(t('common.error'), t('contacts.pinInvalid'));
            return;
        }
        ...
        try {
            ...
            if (res?.success === false) {
                Alert.alert(t('common.error'), res?.message || t('chat.cannotOpen'));
                return;
            }
            ...
            Alert.alert(t('common.error'), e?.message || t('chat.cannotOpen'));
        } finally {
			...
			{Platform.OS === 'web' && (
				<View className="py-4 justify-between flex-row items-center">
					...
					<TouchableOpacity 
						...
						accessibilityLabel={t('qr.confirmLoginTitle')}
					>
							...
							>
								...
								<View className="flex-1 ml-3">
									<Text className="text-amber-900 font-semibold">
										{t('chat.pendingInboxTitle')}
									</Text>
									<Text className="text-amber-700 text-xs mt-0.5">
										{pendingInboxConversations.length} {t('chat.pendingInboxCount')}
									</Text>
								</View>
								...
								>
									...
									<View className="flex-1 ml-3">
										...
										<Text className="text-sm text-amber-700" numberOfLines={1}>
											{t('chat.hiddenConversationPrompt')}
										</Text>
									</View>
							...
							<View className="flex-1 ml-3">
								<View className="flex-row justify-between items-center">
									<View className="flex-row items-center flex-1 mr-2 min-w-0">
										<Text className="font-semibold text-gray-900 text-base flex-shrink" numberOfLines={1}>
											{getConversationName(conversation)}
										</Text>
										{isPrivate && (
											<View
											...
											>
												...
												>
													{isFriend ? t('contacts.alreadyFriend') : t('chat.stranger')}
												</Text>
											</View>
										)}
										{isPinned && (
											<Ionicons name="bookmark" size={13} color="#d97706" style={{ marginLeft: 6 }} />
										)}
										{isMuted && (
											<Ionicons name="notifications-off-outline" size={13} color="#9ca3af" style={{ marginLeft: 6 }} />
										)}
									</View>
                ...
                <View className="flex-1 bg-black/40 items-center justify-center px-5">
                    <View className="w-full max-w-md bg-white rounded-2xl overflow-hidden">
                        <View className="px-5 py-4 border-b border-gray-100">
                            <Text className="text-lg font-semibold text-gray-900">{t('chat.unlockConversationTitle')}</Text>
                            <Text className="text-sm text-gray-500 mt-1">
                                {t('chat.unlockConversationDesc')}
                            </Text>
                        </View>
                        ...
                        <View className="flex-row px-5 pb-4">
                            ...
                            >
                                <Text className="text-white font-semibold">
                                    {unlockLoading ? t('chat.opening') : t('chat.reopen')}
                                </Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                ...
                <View className="flex-1 bg-black/40 items-center justify-center px-5">
                    <View className="w-full max-w-lg bg-white rounded-2xl overflow-hidden">
                        <View className="px-5 py-4 border-b border-gray-100 flex-row items-center justify-between">
                            <View>
                                <Text className="text-lg font-semibold text-gray-900">{t('chat.pendingInboxModalTitle')}</Text>
                                <Text className="text-sm text-gray-500 mt-1">
                                    {t('chat.pendingInboxModalDesc')}
                                </Text>
                            </View>
                            <TouchableOpacity
                        ...
                        <ScrollView className="max-h-[420px]">
                            {pendingInboxConversations.length === 0 ? (
                                <View className="px-5 py-6">
                                    <Text className="text-gray-500 text-center">{t('chat.noPendingConversations')}</Text>
                                </View>
                            ) : (

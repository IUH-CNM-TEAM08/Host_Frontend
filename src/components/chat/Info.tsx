...
function roleLabel(role?: string): string {
    ...
    if (r === 'MODERATOR' || r === 'MOD') return 'Phó nhóm';
    return 'Thành viên';
}
...
export default function Info({
    ...
    const [sharedFiles, setSharedFiles] = useState<{name:string;size:string;type:string;date:string;url?:string}[]>([]);
        ...
        const loadConversationDetail = async () => {
            try {
                ...
                const enrichedParticipantInfo = participantIds.map((id) => {
                    ...
                    return {
                        id,
                        name: current?.name || profile?.name || 'Người dùng',
    ...
    const performLeaveGroup = useCallback(
        async (conversationId: string) => {
            try {
                ...
                return { success: false, message: res.message || 'Không thể rời nhóm' };
            } catch (error) {
                console.error('Error leaving group:', error);
                return { success: false, message: 'Không thể rời nhóm. Vui lòng thử lại.' };
            }
        },
        [onDeleteChat, onBackPress],
    );

    const handleOwnerLeavePress = useCallback(() => {
        if (!selectedChat?.id) return;
        if (ownerTransferCandidates.length === 0) {
            const msg = 'Không còn thành viên để chuyển quyền. Bạn hãy giải tán nhóm thay vì rời nhóm.';
            if (Platform.OS === 'web') {
                window.alert(msg);
            } else {
                Alert.alert('Không thể rời nhóm', msg);
            }
            return;
        }
    ...
    }, [selectedChat?.id, ownerTransferCandidates]);
    ...
    const handleTransferAndLeave = useCallback(async () => {
        ...
        try {
            ...
            if (!transferRes.success) {
                const msg = transferRes.message || 'Không thể chuyển quyền trưởng nhóm.';
                if (Platform.OS === 'web') {
                    window.alert(msg);
                } else {
                    Alert.alert('Lỗi', msg);
            ...
            if (!leaveRes.success) {
                const msg = leaveRes.message || 'Không thể rời nhóm.';
                if (Platform.OS === 'web') {
                    window.alert(`Đã chuyển quyền trưởng nhóm nhưng chưa rời nhóm: ${msg}`);
                } else {
                    Alert.alert('Lỗi', `Đã chuyển quyền trưởng nhóm nhưng chưa rời nhóm: ${msg}`);
                }
                return;
            }
    ...
    }, [selectedChat?.id, ownerLeaveTargetUserId, ownerLeaveSubmitting, performLeaveGroup]);
    ...
    const handleGroupAvatarPress = useCallback(async () => {
        ...
        try {
            ...
            Alert.alert('Lỗi', e?.message || 'Không thể cập nhật ảnh nhóm.');
        } finally {
            setGroupAvatarUploading(false);
        }
    }, [selectedChat?.isGroup, selectedChat?.id, canEditGroupAvatar, onConversationMetaChanged]);
    ...
    const handleDisbandGroup = async () => {
        ...
        const runDisband = async () => {
            ...
            if (!res.success) {
                const msg = res.message || 'Không thể giải tán nhóm.';
                if (Platform.OS === 'web') {
                    window.alert(msg);
                } else {
                    Alert.alert('Lỗi', msg);
        ...
        try {
            ...
            if (Platform.OS === 'web') {
                const ok = window.confirm(
                    'Bạn có chắc muốn giải tán nhóm này không? Hành động này không thể hoàn tác.',
                );
                if (!ok) return;
                await runDisband();
                return;
            ...
            const msg = error?.message || 'Không thể giải tán nhóm. Vui lòng thử lại.';
            if (Platform.OS === 'web') {
                window.alert(msg);
            } else {
                Alert.alert('Lỗi', msg);
    ...
    const handleLeaveGroup = () => {
        ...
        const runLeave = async () => {
            ...
            if (!res.success) {
                if (Platform.OS === 'web') {
                    window.alert(res.message || 'Không thể rời nhóm');
                } else {
                    Alert.alert('Lỗi', res.message || 'Không thể rời nhóm');
                }
            }
        };

        if (Platform.OS === 'web') {
            if (window.confirm('Bạn có chắc muốn rời khỏi nhóm này không?')) void runLeave();
            return;
        }
    ...
    const displayName = selectedChat?.isGroup
        ? (loadConversation?.name || 'Nhóm')
        : (otherUser?.name || loadConversation?.name || 'Người dùng');
                ...
                {selectedChat.isGroup && isGroupOwner && (
                    <View className="mb-2 mx-4 mt-2">
                        ...
                        >
                            ...
                            <Text className="text-red-500 font-semibold text-sm ml-2">Giải tán nhóm</Text>
                        </TouchableOpacity>
                    </View>
                )}
                {selectedChat.isGroup && (!isAdmin || isGroupOwner) && (
                    <View className="mb-4 mx-4 mt-2">
                        ...
                        >
                            ...
                            <Text className="text-orange-500 font-semibold text-sm ml-2">Rời nhóm</Text>
                        </TouchableOpacity>
                    </View>
                )}
            </ScrollView>
                ...
                <View className="flex-1 bg-black/40 items-center justify-center px-5">
                    <View className="w-full max-w-md bg-white rounded-2xl overflow-hidden">
                        <View className="px-5 py-4 border-b border-gray-100">
                            <Text className="text-lg font-semibold text-gray-900">Chuyển quyền trước khi rời nhóm</Text>
                            <Text className="text-sm text-gray-500 mt-1">
                                Chọn thành viên sẽ làm trưởng nhóm mới, sau đó hệ thống sẽ rời nhóm cho bạn.
                            </Text>
                        </View>
                            ...
                            {ownerTransferCandidates.length === 0 ? (
                                <View className="px-5 py-6">
                                    <Text className="text-gray-500 text-center">
                                        Không có thành viên khả dụng để chuyển quyền.
                                    </Text>
                                </View>
                            ) : (
                        ...
                        <View className="flex-row px-5 py-4 border-t border-gray-100">
                            ...
                            >
                                {ownerLeaveSubmitting ? (
                                    ...
                                    <Text style={{ color: '#ffffff', fontWeight: '600' }}>Chuyển quyền và rời nhóm</Text>
                                )}
                            </TouchableOpacity>
                        </View>
                    </View>
                    ...
                    <View className="bg-white rounded-t-3xl max-h-[90%]">
                        <View className="flex-row items-center justify-between px-4 py-4 border-b border-gray-200">
                            <Text className="text-lg font-semibold text-gray-900">
                                {viewAllMode === 'media' ? 'Tất cả ảnh/video' : 'Tất cả file'}
                            </Text>
                            <TouchableOpacity onPress={() => setViewAllMode(null)} className="px-3 py-1 rounded-lg bg-gray-100">
                                <Text className="text-sm text-gray-700">Đóng</Text>
                            </TouchableOpacity>
                        </View>
                        <ScrollView className="px-4 pb-6">
                            {viewAllMode === 'media' ? (
                                mediaUrls.length === 0 ? (
                                    <Text className="text-gray-400 text-center py-6">Chưa có ảnh/video nào</Text>
                                ) : (
                                ...
                                sharedFiles.length === 0 ? (
                                    <Text className="text-gray-400 text-center py-6">Chưa có file nào</Text>
                                ) : (

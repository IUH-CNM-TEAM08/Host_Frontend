...
export default function MediaInfo({ images, onViewAll, loading }: MediaInfoProps) {
    return (
        <View className="px-4 pt-6 pb-4">
            ...
            {loading ? (
                ...
                <Text className="text-gray-400 text-sm text-center py-2">Chưa có ảnh/video nào</Text>
            ) : (
                <View className="flex-row flex-wrap -mx-1">
                    {images.slice(0, 6).map((uri, index) => (
                        <TouchableOpacity key={index} className="w-[32%] aspect-square p-1">
                            <Image
                                source={{ uri }}

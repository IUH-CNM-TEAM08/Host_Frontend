  ...
  return (
    <Modal visible transparent animationType="fade" onRequestClose={onClose}>
      <View style={styles.menuModalRoot}>
        <Pressable style={styles.menuBackdrop} onPress={onClose} accessibilityRole="button" accessibilityLabel={t('common.close')} />
        <View
          style={[
            styles.menuPanel,
            {
      ...
      <TouchableOpacity
        ...
        <Text style={{ marginLeft: 8, fontSize: 14, color: '#374151' }}>{t('groupInfo.viewProfile')}</Text>
      </TouchableOpacity>
      ...
      <TouchableOpacity
        ...
        <Text style={{ marginLeft: 8, fontSize: 14, color: '#374151' }}>{t('groupInfo.setNickname')}</Text>
      </TouchableOpacity>
      <View style={{ height: 1, backgroundColor: '#F3F4F6' }} />
      ...
      {!isAdmin && !isModerator && (
        <>
          <TouchableOpacity
            ...
            <Text style={{ marginLeft: 8, fontSize: 14, color: '#374151' }}>{t('groupInfo.transferAdmin')}</Text>
          </TouchableOpacity>
          <View style={{ height: 1, backgroundColor: '#F3F4F6' }} />
          <TouchableOpacity
            style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 12 }}
            ...
            <Text style={{ marginLeft: 8, fontSize: 14, color: '#374151' }}>{t('groupInfo.addModerator')}</Text>
          </TouchableOpacity>
          <View style={{ height: 1, backgroundColor: '#F3F4F6' }} />
          <TouchableOpacity
            style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 12 }}
            ...
            <Text style={{ marginLeft: 8, fontSize: 14, color: '#EF4444' }}>{t('groupInfo.removeFromGroup')}</Text>
          </TouchableOpacity>
          <View style={{ height: 1, backgroundColor: '#F3F4F6' }} />
          <TouchableOpacity
            style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 12 }}
            ...
            <Text style={{ marginLeft: 8, fontSize: 14, color: '#DC2626' }}>{t('groupInfo.banFromGroup')}</Text>
          </TouchableOpacity>
        </>
      )}
      {isModerator && onRevokeMod && (
        <>
          <TouchableOpacity
            ...
            <Text style={{ marginLeft: 8, fontSize: 14, color: '#EF4444' }}>{t('groupInfo.revokeModerator')}</Text>
          </TouchableOpacity>
          <View style={{ height: 1, backgroundColor: '#F3F4F6' }} />
        </>
      )}
      {isAdmin && onRemoveAdmin && (
        <TouchableOpacity
          style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 12 }}
          onPress={() => { onRemoveAdmin?.(memberId); onClose(); }}>
          <Ionicons name="trash-outline" size={18} color="#EF4444" />
          <Text style={{ marginLeft: 8, fontSize: 14, color: '#EF4444' }}>{t('groupInfo.removeAdminRole')}</Text>
        </TouchableOpacity>
      )}
      {!isAdmin && isModerator && (
        <>
...
export default function GroupInfo({ group, onParticipantsSynced }: GroupInfoProps) {
  ...
  const canManageJoinRequests = isAdmin || (isMod && isAllowModManage);

  // ── Load pending join requests (admin/owner hoặc phó nhóm khi được cấp quyền) ──────────────────
  const loadPendingRequests = useCallback(async () => {
    console.log('[GroupInfo] loadPendingRequests called', { canManageJoinRequests, requireApproval, groupId: group.id, myRole });
    if (!canManageJoinRequests || !requireApproval) { setPendingRequests([]); return; }
  ...
  }, [group.id, canManageJoinRequests, requireApproval, myRole]);
      ...
      if (res.success && res.conversation) {
        ...
        setMembers((prev) =>
          list.map((p) => {
            ...
            return {
              ...p,
              name: nameOk ? p.name : old?.name || p.name || t('chatArea.defaultUser'),
  ...
  useEffect(() => {
    ...
    const onJoinRequest = (data: any) => {
      ...
      const requesterName = String(data?.requesterName ?? '').trim() || t('chatArea.defaultUser');
  ...
  const handleRemoveMember = (memberId: string) => {
    confirmDestructive(
      t('groupInfo.removeMemberTitle'),
      t('groupInfo.removeMemberConfirm'),
      t('groupInfo.remove'),
      async () => {
        ...
        if (res.success) {
          ...
          showErrorAlert(t('common.error'), res.message || t('groupInfo.cannotRemoveMember'));
        }
      },
    );
  };
  ...
  const handleBanMember = (memberId: string) => {
    confirmDestructive(
      t('groupInfo.banMemberTitle'),
      t('groupInfo.banMemberConfirm'),
      t('groupInfo.ban'),
      async () => {
        ...
        showErrorAlert(t('common.error'), res.message || t('groupInfo.cannotBanMember'));
      },
    );
  };
  ...
  const handleUnbanMember = (memberId: string) => {
    confirmDestructive(
      t('groupInfo.unbanMemberTitle'),
      t('groupInfo.unbanMemberConfirm'),
      t('groupInfo.unban'),
      async () => {
        ...
        showErrorAlert(t('common.error'), res.message || t('groupInfo.cannotUnbanMember'));
      },
    );
  };
  ...
  const handleAddAdmin = async (memberId: string) => {
    ...
    if (!res.success) showErrorAlert(t('common.error'), res.message || t('groupInfo.cannotTransferAdmin'));
    refreshParticipants();
  };
  ...
  const handleAddMod = async (memberId: string) => {
    ...
    if (!res.success) showErrorAlert(t('common.error'), res.message || t('groupInfo.cannotGrantMod'));
    refreshParticipants();
  };
  ...
  const handleRevokeMod = async (memberId: string) => {
    ...
    if (!res.success) {
      setMembers(previous);
      showErrorAlert(t('common.error'), res.message || t('groupInfo.cannotRevokeMod'));
      return;
    }
  ...
  const handleRemoveAdmin = (memberId: string) => {
    confirmDestructive(
      t('groupInfo.downgradeTitle'),
      t('groupInfo.downgradeConfirm'),
      t('groupInfo.confirm'),
      async () => {
        setMembers((prev) =>
          prev.map((m) => (m.id === memberId ? { ...m, role: "member" as const } : m)),
        ...
        const res = await ConversationService.revokeModRole(group.id, memberId);
        if (!res.success) showErrorAlert(t('common.error'), res.message || t('groupInfo.cannotDowngrade'));
        refreshParticipants();
      },
    );
  };
  ...
  const submitNickname = async () => {
    ...
    try {
      ...
      if (!res.success) {
        Alert.alert(t('common.error'), res.message || t('groupInfo.cannotUpdateNickname'));
        return;
      }
      ...
      Alert.alert(t('common.error'), t('groupInfo.cannotUpdateNickname'));
    } finally {
  ...
  // ── Load pending join requests (theo quyền quản lý thành viên) ──────────────────

  /** Tải yêu cầu chờ duyệt khi có quyền — không phụ thuộc mở/đóng section (tránh mất số liệu khi đóng/mở lại panel). */
  useEffect(() => {
    if (canManageJoinRequests && requireApproval) void loadPendingRequests();
  }, [canManageJoinRequests, requireApproval, loadPendingRequests, group.id]);
  ...
  }, [showDetail, canManageJoinRequests, isAdmin, isMod, requireApproval, loadPendingRequests, loadBlockedMembers]);

  const handleApproveRequest = async (inviteId: string, name: string) => {
    try {
      await ConversationService.approveJoinRequest(inviteId);
      Alert.alert(t('groupInfo.approvedTitle'), t('groupInfo.approvedMessage').replace('{name}', name));
      setPendingRequests(prev => prev.filter(r => String(r._id ?? r.id ?? '') !== String(inviteId)));
      refreshParticipants();
    } catch { Alert.alert(t('common.error'), t('groupInfo.cannotApproveRequest')); }
  };
    ...
    const performReject = async () => {
      try {
        ...
        if (res.success) {
          ...
          Alert.alert(t('common.error'), res.message || t('groupInfo.cannotRejectRequest'));
        }
      } catch (e) {
        console.error('[GroupInfo] reject error:', e);
        Alert.alert(t('common.error'), t('groupInfo.cannotRejectRequest'));
      }
    };

    if (Platform.OS === 'web') {
      if (window.confirm(t('groupInfo.rejectRequestConfirm').replace('{name}', name))) {
        void performReject();
      }
    } else {
      Alert.alert(t('groupInfo.reject'), t('groupInfo.rejectRequestPrompt').replace('{name}', name), [
        { text: t('common.cancel'), style: 'cancel' },
        { text: t('groupInfo.reject'), style: 'destructive', onPress: performReject },
      ]);
    }
  };
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              {isMyself && (
                <View style={{ marginLeft: 6, backgroundColor: '#6366F1', borderRadius: 4, paddingHorizontal: 5, paddingVertical: 1 }}>
                  <Text style={{ fontSize: 10, color: '#fff', fontWeight: '700' }}>{t('groupInfo.you')}</Text>
                </View>
              )}
            </View>
              ...
              {(() => {
                ...
                if (r === 'OWNER') return (
                  <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#FEF3C7', borderRadius: 6, paddingHorizontal: 6, paddingVertical: 2 }}>
                    ...
                    <Text style={{ fontSize: 11, color: '#D97706', marginLeft: 3, fontWeight: '700' }}>{t('groupInfo.owner')}</Text>
                  </View>
                );
                if (r === 'ADMIN') return (
                  <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#DBEAFE', borderRadius: 6, paddingHorizontal: 6, paddingVertical: 2 }}>
                    ...
                    <Text style={{ fontSize: 11, color: '#2563EB', marginLeft: 3, fontWeight: '600' }}>{t('groupInfo.admin')}</Text>
                  </View>
                );
                if (r === 'MOD' || r === 'MODERATOR') return (
                  <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#D1FAE5', borderRadius: 6, paddingHorizontal: 6, paddingVertical: 2 }}>
                    ...
                    <Text style={{ fontSize: 11, color: '#059669', marginLeft: 3, fontWeight: '600' }}>{t('groupInfo.moderator')}</Text>
                  </View>
                );
                return (
                  <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#F3F4F6', borderRadius: 6, paddingHorizontal: 6, paddingVertical: 2 }}>
                    <Text style={{ fontSize: 11, color: '#6B7280' }}>{t('groupInfo.member')}</Text>
                  </View>
                );
              })()}
            </View>
    ...
    <View style={{ paddingHorizontal: 16, paddingTop: 16, paddingBottom: 16, borderBottomWidth: 4, borderBottomColor: '#E5E7EB', overflow: 'visible' }}>
      ...
      <TouchableOpacity
        ...
        <Text style={{ fontSize: 15, fontWeight: '600', color: '#1e293b' }}>
          {t('groupInfo.groupMembers')} ({members.length})
        </Text>
      ...
      {showDetail && (
        <View style={{ marginTop: 4 }}>
          {/* Yêu cầu chờ duyệt — bật duyệt thành viên mới + có quyền quản lý thành viên */}
          {requireApproval && canManageJoinRequests && (
            <View
              style={{
                marginBottom: 12,
                paddingBottom: 12,
            ...
            >
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
                ...
                >
                  {t('groupInfo.pendingRequests').toUpperCase()} ({pendingRequests.length})
                </Text>
              </View>
              {loadingPending ? (
                <ActivityIndicator size="small" color="#D97706" />
              ) : pendingRequests.length === 0 ? (
                <Text style={{ fontSize: 13, color: '#9CA3AF' }}>{t('groupInfo.noPendingRequests')}</Text>
              ) : (
                pendingRequests.map((req) => {
                  const inviteId = String(req.id ?? req._id ?? '');
                  const u = req.user;
                  const displayName = u?.name || req.invitee?.name || t('chatArea.defaultUser');
                  ...
                  return (
                    ...
                    >
                      ...
                      <View style={{ flex: 1 }}>
                        ...
                        <Text style={{ fontSize: 11, color: '#9CA3AF', marginTop: 2 }}>{t('groupInfo.awaitingApproval')}</Text>
                      </View>
                      <TouchableOpacity
                      ...
                      >
                        <Text style={{ color: '#fff', fontSize: 12, fontWeight: '600' }}>{t('groupInfo.approve')}</Text>
                      </TouchableOpacity>
                      <TouchableOpacity
                      ...
                      >
                        <Text style={{ color: '#4B5563', fontSize: 12, fontWeight: '600' }}>{t('groupInfo.reject')}</Text>
                      </TouchableOpacity>
                    </View>
                  );
                })
          ...
          {(isAdmin || isMod) && (
            ...
            >
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
                ...
                >
                  {t('groupInfo.blockedList').toUpperCase()} ({blockedMembers.length})
                </Text>
              </View>
              {loadingBlocked ? (
                <ActivityIndicator size="small" color="#DC2626" />
              ) : blockedMembers.length === 0 ? (
                <Text style={{ fontSize: 13, color: '#9CA3AF' }}>{t('groupInfo.noBlockedMembers')}</Text>
              ) : (
                blockedMembers.map((bm) => {
                  const blockedId = String(bm.id || bm.userId || '');
                  const displayName =
                    String(bm.name || bm.displayName || t('chatArea.defaultUser')).trim() || t('chatArea.defaultUser');
                  ...
                  return (
                    ...
                    >
                      ...
                      <View style={{ flex: 1 }}>
                        <Text style={{ fontSize: 14, fontWeight: '600', color: '#111827' }}>{displayName}</Text>
                        <Text style={{ fontSize: 11, color: '#9CA3AF', marginTop: 2 }}>{t('groupInfo.bannedFromGroupStatus')}</Text>
                      </View>
                      <TouchableOpacity
                      ...
                      >
                        <Text style={{ color: '#166534', fontSize: 12, fontWeight: '700' }}>{t('groupInfo.unban')}</Text>
                      </TouchableOpacity>
                    </View>
                  );
                })
            ...
            <View style={{ marginBottom: 8 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 6 }}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#FEF3C7', borderRadius: 20, paddingHorizontal: 8, paddingVertical: 3, marginHorizontal: 8 }}>
                  <Text style={{ fontSize: 10, color: '#D97706', fontWeight: '800', letterSpacing: 0.5 }}>⭐ {t('groupInfo.owner').toUpperCase()}</Text>
                </View>
                <View style={{ flex: 1, height: 1, backgroundColor: '#F3F4F6' }} />
              </View>
            ...
            <View style={{ marginBottom: 8 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 6 }}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#DBEAFE', borderRadius: 20, paddingHorizontal: 8, paddingVertical: 3, marginHorizontal: 8 }}>
                  ...
                  <Text style={{ fontSize: 10, color: '#2563EB', fontWeight: '800', letterSpacing: 0.5, marginLeft: 3 }}>{t('groupInfo.admin').toUpperCase()} ({admins.length})</Text>
                </View>
                <View style={{ flex: 1, height: 1, backgroundColor: '#F3F4F6' }} />
              </View>
            ...
            <View style={{ marginBottom: 8 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 6 }}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#D1FAE5', borderRadius: 20, paddingHorizontal: 8, paddingVertical: 3, marginHorizontal: 8 }}>
                  ...
                  <Text style={{ fontSize: 10, color: '#059669', fontWeight: '800', letterSpacing: 0.5, marginLeft: 3 }}>{t('groupInfo.moderator').toUpperCase()} ({mods.length})</Text>
                </View>
                <View style={{ flex: 1, height: 1, backgroundColor: '#F3F4F6' }} />
              </View>
            ...
            <View>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 6 }}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#F3F4F6', borderRadius: 20, paddingHorizontal: 8, paddingVertical: 3, marginHorizontal: 8 }}>
                  ...
                  <Text style={{ fontSize: 10, color: '#6B7280', fontWeight: '700', letterSpacing: 0.5, marginLeft: 3 }}>{t('groupInfo.member').toUpperCase()} ({normalMembers.length})</Text>
                </View>
                <View style={{ flex: 1, height: 1, backgroundColor: '#F3F4F6' }} />
              </View>
        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.4)', justifyContent: 'center', alignItems: 'center' }}>
          <View style={{ backgroundColor: '#fff', borderRadius: 16, width: '85%', maxWidth: 380, overflow: 'hidden' }}>
            <View style={{ padding: 20, borderBottomWidth: 1, borderBottomColor: '#F3F4F6' }}>
              <Text style={{ fontSize: 16, fontWeight: '700', color: '#111827' }}>{t('groupInfo.setNickname')}</Text>
              <Text style={{ fontSize: 13, color: '#6B7280', marginTop: 4 }}>
                {t('groupInfo.changeNicknameFor').replace('{name}', nicknameTarget?.name || t('groupInfo.member').toLowerCase())}
              </Text>
            </View>
            ...
            <View style={{ flexDirection: 'row', padding: 16, borderTopWidth: 1, borderTopColor: '#F3F4F6', gap: 10 }}>
              ...
              <TouchableOpacity
                ...
                <Text style={{ fontSize: 15, fontWeight: '600', color: '#fff' }}>
                  {nicknameSaving ? t('groupInfo.saving') : t('groupInfo.save')}
                </Text>
              </TouchableOpacity>
            </View>
          </View>

...
export default function GroupManagementModal({ visible, onClose, selectChat }: GroupManagementModalProps) {
    ...
    // Sync local toggles whenever group settings change while modal is visible.
    useEffect(() => {
        if (!visible) return;
        setSettings({
        ...
        visible,
        selectChat?.id,
        selectChat?.settings?.isAllowMessaging,
        selectChat?.settings?.isAllowMemberChangeMetadata,
        selectChat?.settings?.isAllowMemberPin,
        selectChat?.settings?.isAllowMemberCreateNote,
        selectChat?.settings?.isAllowMemberCreatePoll,
        selectChat?.settings?.isAllowModManage,
    ]);

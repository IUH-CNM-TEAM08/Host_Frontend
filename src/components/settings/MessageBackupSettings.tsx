...
export default function MessageBackupSettings() {
  ...
  const handleExport = async () => {
    if (!user?.id) {
      Alert.alert(t('common.error'), t('messageBackup.noUserFound'));
      return;
    }
    ...
    try {
      ...
      Alert.alert(t('common.error'), t('messageBackup.exportFailed'));
    } finally {
      setExporting(false);
    }
  };
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (!resultUri) {
        ...
        Alert.alert(t('common.error'), t('messageBackup.cannotSelectFile'));
        return;
      }

      if (!resultName?.toLowerCase().endsWith('.zip')) {
        console.log('[MessageBackupSettings] invalid file extension', resultName);
        Alert.alert(t('common.error'), t('messageBackup.selectZipFile'));
        return;
      }
      ...
      if (res?.success) {
        ...
        Alert.alert(t('common.error'), res?.message || t('messageBackup.importFailed'));
      }
    } catch (error) {
      console.error('[MessageBackupSettings] import error', error);
      Alert.alert(t('common.error'), t('messageBackup.importFailed'));
    } finally {
      setImporting(false);
  ...
  return (
    <View className="border-b border-gray-200 pb-6">
      <View className="flex-row justify-between items-center mb-4">
        <View>
          <Text className="text-lg font-medium text-gray-900">{t('messageBackup.title')}</Text>
          <Text className="text-sm text-gray-500">{t('messageBackup.subtitle')}</Text>
        </View>
      </View>
      ...
      <View className="flex-row flex-wrap gap-3">
        ...
        >
          <Text className={`font-medium ${exporting ? 'text-blue-600' : 'text-blue-700'}`}>
            {exporting ? t('messageBackup.exporting') : t('messageBackup.export')}
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
        ...
        >
          <Text className={`font-medium ${importing ? 'text-green-600' : 'text-green-700'}`}>
            {importing ? t('messageBackup.importing') : t('messageBackup.import')}
          </Text>
        </TouchableOpacity>
      </View>
      {importing && (
        <View className="mt-3 flex-row items-center space-x-2">
          ...
          <Text className="text-sm text-gray-600">
            {t('messageBackup.restoring')}{importingProgress != null ? ` (${importingProgress}%)` : '...'}
          </Text>
        </View>
      )}

...
function StoryBubble({ story, isOwn, onPress }: StoryBubbleProps) {
  ...
  const mediaTypeNorm = String(story?.mediaType ?? story?.type ?? '').toLowerCase();
  const isVideoStory = mediaTypeNorm === 'video' || /\.(mp4|mov|webm|ogg)$/i.test(mediaUri);
  const isTextStory = !mediaUri || mediaTypeNorm === 'text';
    ...
    >
      {isOwn ? (
        ...
        <View className="w-full h-full">
          {isVideoStory ? (
            <View className="w-full h-full" style={{ backgroundColor: '#0f172a' }}>
              <Ionicons name="play-circle-outline" size={34} color="#93c5fd" style={{ alignSelf: 'center', marginTop: 58 }} />

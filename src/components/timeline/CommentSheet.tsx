...
function timeAgo(date?: string, t?: any): string {
  ...
  if (m < 1) return t ? t('commentSheet.justNow') : 'Vừa xong';
  if (m < 60) return t ? m + ' ' + t('commentSheet.minutesAgo') : `${m} phút trước`;
  const h = Math.floor(m / 60);
  if (h < 24) return t ? h + ' ' + t('commentSheet.hoursAgo') : `${h} giờ trước`;
  const d = Math.floor(h / 24);
  return d < 7 ? (t ? d + ' ' + t('commentSheet.daysAgo') : `${d} ngày trước`) : new Date(date).toLocaleDateString('vi-VN');
}
...
function normalizeComment(raw: any, t?: any): Comment {
  ...
  const comment: Comment = {
    ...
    authorName: raw?.author?.name ?? raw?.author?.displayName ?? raw?.authorName ?? (t ? t('commentSheet.anonymous') : 'Ẩn danh'),
    authorAvatar: avatarUrl,
    content: raw?.content ?? '',
    createdAt: raw?.createdAt,
    replies: Array.isArray(raw?.replies) ? raw.replies.map(r => normalizeComment(r, t)) : undefined,
  };
    ...
    <View style={{ marginTop: 10, padding: 12, backgroundColor: '#eef2ff', borderRadius: 14 }}>
      {targetAuthorName ? (
        <View style={{ marginBottom: 10, padding: 10, backgroundColor: '#fff', borderRadius: 14, borderWidth: 1, borderColor: '#dbeafe' }}>
          <Text style={{ fontSize: 12, color: '#3730a3', fontWeight: '700' }}>{t('commentSheet.replyingTo')} {targetAuthorName}</Text>
          {targetSnippet ? (
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'flex-end', gap: 10, marginTop: 10 }}>
        <TouchableOpacity onPress={onCancel} style={{ paddingVertical: 6, paddingHorizontal: 12 }}>
          <Text style={{ color: '#6d28d9', fontWeight: '700' }}>{t('common.cancel')}</Text>
        </TouchableOpacity>
        <TouchableOpacity
          onPress={onSubmit}
        ...
        >
          <Text style={{ color: value.trim() && !submitting ? '#fff' : '#9ca3af', fontWeight: '700' }}>
            {submitting ? t('commentSheet.sending') : t('common.send')}
          </Text>
        </TouchableOpacity>
      </View>
    </View>
    ...
    <View style={{ flexDirection: 'row', gap: 9, paddingHorizontal: 14, paddingVertical: 6 }}>
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 14, marginTop: 3, paddingLeft: 4 }}>
          <Text style={{ fontSize: 11, color: '#9ca3af' }}>{timeAgo(comment.createdAt, t)}</Text>
          <TouchableOpacity onPress={() => onReply(comment)}>
            <Text style={{ fontSize: 11, color: '#6d28d9', fontWeight: '600' }}>{t('commentSheet.replyAction')}</Text>
          </TouchableOpacity>
          {isOwner && (
            <TouchableOpacity onPress={() => {
              Alert.alert(t('commentSheet.deleteConfirmTitle'), '', [
                { text: t('common.cancel'), style: 'cancel' },
                { text: t('common.delete'), style: 'destructive', onPress: async () => {
                  try { await commentService.delete(comment.id); onDelete(comment.id); }
                  catch { Alert.alert(t('common.error'), t('commentSheet.deleteError')); }
                }},
              ]);
            }}>
...
function ReplyItem({ reply, currentUserId, onDelete, onReply, replyToId, replyText, onChangeReplyText, onSubmitReply, onCancelReply, submittingReply }: { reply: any; currentUserId?: string; onDelete: (id: string) => void; onReply: (comment: any) => void; replyToId?: string; replyText?: string; onChangeReplyText?: (text: string) => void; onSubmitReply?: () => void; onCancelReply?: () => void; submittingReply?: boolean }) {
      ...
      <View style={{ flexDirection: 'row', gap: 9, paddingHorizontal: 0, paddingVertical: 0 }}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginTop: 3, paddingLeft: 4 }}>
            <Text style={{ fontSize: 11, color: '#9ca3af' }}>{timeAgo(reply.createdAt, t)}</Text>
...
export default function CommentSheet({ visible, post, onClose, onCommentCountChanged }: CommentSheetProps) {
  ...
  const loadComments = useCallback(async () => {
    ...
    try {
      ...
      const normalized = list.map(item => normalizeComment(item, t));
  ...
  const handleSubmitReply = async () => {
    ...
    try {
      ...
      const newComment = normalizeComment({
        ...raw,
        authorName: user?.displayName ?? user?.name ?? t('commentSheet.you'),
      ...
      Alert.alert(t('common.error'), e?.response?.data?.message ?? t('commentSheet.replyFailed'));
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert(t('common.error'), e?.response?.data?.message ?? t('commentSheet.commentFailed'));
    } finally {
      setSubmitting(false);
    }
  };
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <CommentItem
                  ...
                  onReply={(comment) => {
                    setReplyText('');
                    setReplyTo({ id: comment.id, authorName: comment.authorName ?? t('commentSheet.user'), content: comment.content });
                  }}
            ...
            <View style={{
              ...
              <View style={{
                ...
                <TextInput
                  ...
                  placeholder={t('commentSheet.writeCommentPlaceholder')}

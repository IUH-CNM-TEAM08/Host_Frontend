...
export default function CreateStoryModal({ visible, onClose, onCreated }: CreateStoryModalProps) {
  ...
  const [mediaExt, setMediaExt] = useState<string>('jpg');
  ...
  const getExtFromMime = (mime: string | null) => {
    if (!mime) return '';
    const m = /^video\/(.+)$/.exec(mime.toLowerCase());
    if (m) {
      const subtype = m[1];
    ...
    const m2 = /^image\/(.+)$/.exec(mime.toLowerCase());
    if (m2) return m2[1].includes('png') ? 'png' : 'jpg';
    return '';
  };
  ...
  const pickMedia = async () => {
    ...
    if (!result.canceled && result.assets?.[0]) {
      ...
      setMediaMimeType(detectedMime);

      const extFromUri = getExtFromUri(asset.uri);
      const extFromMime = getExtFromMime(detectedMime);
      const finalExt = extFromUri || extFromMime || (detectedType === 'VIDEO' ? 'mp4' : 'jpg');
      setMediaExt(finalExt);
      if (Platform.OS === 'web') {
        ...
        <View style={{ flex: 1, backgroundColor: Platform.OS === 'web' ? 'rgba(0,0,0,0.8)' : '#000', justifyContent: 'center', alignItems: 'center' }}>
          <View style={{ 
            ...
            <View style={{ flex: 1, backgroundColor: mediaUri ? '#000' : '#1f2937', justifyContent: 'center', alignItems: 'center' }}>
              {mediaUri ? (
            mediaType === 'VIDEO' ? (
              <View style={{ flex: 1, width: '100%', justifyContent: 'center', alignItems: 'center', backgroundColor: '#0f172a' }}>
                <Ionicons name="play-circle-outline" size={56} color="#93c5fd" />
                <Text style={{ color: '#e5e7eb', marginTop: 10, fontSize: 14 }}>Video story</Text>
              </View>
            ) : (

...
export const conversationService = {
  ...
  getByCode: <T = unknown>(code: string, query?: Record<string, unknown>) =>
    get<T>(`/api/conversations/code/${encodeURIComponent(code)}`, query),
  ...
  getConversationByCode: async (code: string, userId?: string) => {
    const query: Record<string, unknown> = {};
    if (userId) query.userId = userId;
    const res: any = await conversationService.getByCode<any>(code, Object.keys(query).length ? query : undefined);
    const conversation = mapApiConversationToModel(unwrapData<any>(res));
    return { success: res?.success ?? true, message: res?.message, conversation, data: conversation };
  },
  ...
  /**
   * Bật/tắt AI cho cuộc trò chuyện.
   */
  updateAiEnabled: async (conversationId: string, aiEnabled: boolean) => {
    try {
      const res: any = await patch<any>(
        `/api/conversations/${encodeURIComponent(conversationId)}/ai-enabled`,
        { aiEnabled },
      );
      const conversation = mapApiConversationToModel(unwrapData<any>(res));
      return {
      ...
      return {
        ...
        message:
          ...
          e?.response?.data?.error ||
          e?.message ||
          'Lỗi cập nhật AI cho cuộc trò chuyện',
      };
    }
  },

  /**
   * Cập nhật các quyền trong màn Quản lý nhóm.
   */
  updateGroupSettings: async (conversationId: string, settings: Record<string, unknown>) => {
    try {
      const res: any = await patch<any>(
  ...
  /**
   * Tương thích ngược cho ChatArea cũ: lưu thứ tự tin nhắn ghim.
   */

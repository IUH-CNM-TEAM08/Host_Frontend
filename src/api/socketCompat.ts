...
class SocketCompat {
  ...
  private ensureSocket() {
    ...
    if (!(s as any).__zalaLifecycle) {
      ...
      const ioSocket = s as any;
      if (typeof ioSocket.onAny === "function") {
        ioSocket.onAny((event: string, ...args: unknown[]) => {
          if (!isDev) return;
          if (event === "message:new" || event === "chat:new-message") {
            const m = args[0] as { type?: string; _id?: string; id?: string };
            zalaLog("recv", event, m?.type, m?._id ?? m?.id);
          }
        });
      }
    }
  ...
  /** Lắng nghe thông báo mới từ server (notification:new) */
  onNotificationNew(cb: (notif: any) => void) {
    this.on('notification:new', cb);
  }

  removeNotificationNewListener(cb: (notif: any) => void) {
  ...
  removeConversationUpdatedListener(cb: Handler) {
    this.off("chat:conversation-updated", cb);
  }
  ...
  /** Lắng nghe khi có thành viên mới được thêm vào nhóm (server broadcast) */
  onParticipantsAddedServer(cb: (data: { conversationId: string; participantIds: string[] }) => void) {
    this.on("conversation:participants_added", cb as Handler);
  }
  removeParticipantsAddedServer(cb: Handler) {
    this.off("conversation:participants_added", cb);
  }
  ...
  /** Lắng nghe khi cài đặt nhóm thay đổi (settings_updated → isAllowMessaging…) */
